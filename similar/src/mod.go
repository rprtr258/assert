...
// in all cases).
func DiffDeadline[O comparable, D DiffHook](
	alg Algorithm,
	d D,
	old []O,
	old_range Range,
	...
	switch alg {
	case AlgorithmMyers:
		return myers_diff_deadline(d, old, old_range, new, new_range, deadline)
	case AlgorithmPatience:
		return patience_diff_deadline(d, old, old_range, new, new_range, deadline)
	case AlgorithmLcs:
		return lcs_diff_deadline(d, old, old_range, new, new_range, deadline)
	default:
		panic("unreachable")
	}
}
