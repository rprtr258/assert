...
func patience_diff_deadline[O comparable, D DiffHook](
	d D,
	old []O, old_range Range,
	new []O, new_range Range,
	deadline Option[Instant],
) Result[Unit] {
	old_indexes := unique(old, old_range)
	new_indexes := unique(new, new_range)

	d2 := NewReplace(Patience{
		d,
		old,
	...
	return myers_diff_deadline(
		d2,
		&old_indexes,
		Range{0, old_indexes.len()},
		&new_indexes,
		Range{0, new_indexes.len()},
		deadline,
	)
}

type Patience[O comparable, D DiffHook] struct {
	d           D
	old         []O
	old_current usize
	old_end     usize
	old_indexes []UniqueItem
	new         []O
	new_current usize
	new_end     usize
	new_indexes []UniqueItem
	deadline    Option[Instant]
}
